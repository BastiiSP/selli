...
Rein visuelle Erweiterung, keine neue Logik. Nutzt für v1 bestehende Maskottchen-Assets als Platzhalter-Posen (`mascot_pushing` fürs Laufen, `mascot_celebrating` fürs Ankommen, `mascot_pondering` für "nichts geplant") — eine eigens produzierte Gehpose ist als optionaler Folge-Task vorgemerkt (siehe unten), kein Blocker für dieses Feature.
...
private fun WirZeitCountdownScene(
    ...
    val mascotDrawable = when (countdown.state) {
        WirZeitCountdownState.WALKING -> R.drawable.mascot_pushing
...
## Task 7 (Optional, nicht blockierend): Asset-Produktion — echte Gehpose statt Platzhalter

**Owner:** Claude (gemeinsam mit Basti, keine Codex-Aufgabe — reine Bildgenerierung über `tools/generate-assets/`, kein Code-Constraint-Vertrag mit anderen Tasks)

Task 5 nutzt `mascot_pushing` (bestehende "schiebende" Pose aus der Wisch-Animation) als Platzhalter für die Wanderweg-Animation. Sobald Zeit ist:

- [ ] Prompt im Stil des bestehenden `STYLE_ANCHOR` aus `tools/generate-assets/generate_assets.py` formulieren (Gehpose, seitliche Ansicht, ein Bein leicht angehoben) — analog zum bisherigen Vorgehen bei `mascot_pondering`/`mascot_pushing` (Referenzbild + Prompt, `rembg`-Freistellung, Transparenz-Check per Pixel-Histogramm, 512×512, `drawable-nodpi`).
- [ ] Neues Asset z. B. als `mascot_walking.webp` einsetzen, `R.drawable.mascot_pushing` in Task 5s `WirZeitCountdownScene` durch `R.drawable.mascot_walking` ersetzen (einzeilige Änderung).
- [ ] `clean`-Build + Tests, `selli.apk` neu bauen, Gerätetest.

Kein eigener Implementierungsplan nötig — reine Asset-Ersetzung, sobald das Bild da ist.

---

## Selbstprüfung (bereits durchgeführt)

- **Spec-Abdeckung:** Alle Abschnitte aus `docs/superpowers/specs/2026-08-05-wir-zeit-countdown-design.md` sind abgedeckt — `created`-Feld (Task 1), Fortschritts-/Text-Logik inkl. aller Randfälle (Task 2), unabhängiger 30-Tage-Fetch (Task 3), Header-Verhalten inkl. Tap/Ticken (Task 4), Wanderweg-Visualisierung (Task 5), Widget-Zeile (Task 6), Lottie/Assets als bewusst zurückgestellt (Task 7 optional).
- **Platzhalter-Scan:** Keine TBD/TODO-Stellen; Task 5s Platzhalter-Assets sind explizit benannt und über Task 7 auflösbar, kein vages "später klären".
